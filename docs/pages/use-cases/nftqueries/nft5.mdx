...
These are on-chain components, where datum and redeemer information are stored  in the Ledger Sync database in CBOR and JSON format in the following tables: [datum](/../../schema.md#datum), [redeemer_data](/../../schema.md#redeemer_data), and also  referenced on [tx_out](/../../schema.md#tx_out) and [failed_tx_out](/../../schema.md#failed_tx_out). 
...
`FROM ma_tx_out mto` selects data from the [ma_tx_out](/../../schema.md#ma_tx_out) table, which stores information about asset transaction outputs.

`JOIN multi_asset ma ON ma.id = mto.ident` joins the [ma_tx_out](/../../schema.md#ma_tx_out) table with the multi_asset table based on matching `id` and `ident` values. This links the transaction output information with the details of the asset itself.

`JOIN tx_out ON mto.tx_out_id = tx_out.id` joins the result with the [tx_out](/../../schema.md#tx_out) table to get the transaction output details associated with the asset outputs.

`LEFT JOIN tx_in ti ON ti.tx_out_id = tx_out.tx_id AND ti.tx_out_index = tx_out."index"` performs a left join with the [tx_in](/../../schema.md#tx_in) table to identify which transaction outputs have been spent (used as inputs in other transactions).
