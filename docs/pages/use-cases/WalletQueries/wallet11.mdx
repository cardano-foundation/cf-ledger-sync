...
The primary focus of the query is to fetch data from the [gov_action_proposal](/../../schema.md#gov_action_proposal) table (alias `gap`). This returns details like:
...
The subquery aliased as `voting` calculates the `yes_vote`, `no_vote`, and `abstain_vote` counts for each proposal. It does this by checking the [voting_procedure](/../../schema.md#voting_procedure) table, filtering for the latest vote from each voter on each proposal `(using ROW_NUMBER() and rank_slot = 1)`. It then groups the results by gov_action_tx_hash and gov_action_index to get totals per proposal.
...
- The latest `gov_action_lifetime` parameter from the [epoch_param](/../../schema.md#epoch_param) table, ie. how long proposals remain active.
- The latest `epoch_no` from the [epoch_param](/../../schema.md#epoch_param) table, ie. the current epoch.
