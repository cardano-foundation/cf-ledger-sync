...
`FROM pool_hash ph` selects all rows from the [pool_hash](https://github.com/cardano-foundation/cf-ledger-sync/blob/main/docs/pages/schema.md#pool_hash) table.

`LEFT JOIN pool_offline_data po on ph.id = po.pool_id and ...` performs a `LEFT JOIN` with [pool_offline_data](https://github.com/cardano-foundation/cf-ledger-sync/blob/main/docs/pages/schema.md#pool_offline_data) to get the pool's name and ticker. The `ON` clause ensures we get the most recent metadata for each pool:
 - `ph.id = po.pool_id` matches rows based on the pool ID.
 - `(po.id is null or po.id = (SELECT max(po2.id) ...))` is a subquery which selects the maximum `id` from [pool_offline_data](https://github.com/cardano-foundation/cf-ledger-sync/blob/main/docs/pages/schema.md#pool_offline_data) for the same `pool_id`.
...
`LEFT JOIN pool_update pu on ph.id = pu.hash_id and ...` performs another `LEFT JOIN` with [pool_update](https://github.com/cardano-foundation/cf-ledger-sync/blob/main/docs/pages/schema.md#pool_update) to get the pool's registration transaction ID. Again, the `ON` clause ensures we get the earliest update for each pool:
 - `ph.id = pu.hash_id` matches rows based on the pool ID.
 - `pu.id = (SELECT min(pu2.id) ...)` is a subquery which selects the minimum `id` from [pool_update](https://github.com/cardano-foundation/cf-ledger-sync/blob/main/docs/pages/schema.md#pool_update) for the same `hash_id`
This allows us find the first update, which is the registration.

`LEFT JOIN tx ON tx.id = pu.registered_tx_id` joins with the [tx](https://github.com/cardano-foundation/cf-ledger-sync/blob/main/docs/pages/schema.md#tx) table to get the transaction details associated with the pool's registration. `LEFT JOIN block b ON tx.block_id = b.id` joins with the [block](https://github.com/cardano-foundation/cf-ledger-sync/blob/main/docs/pages/schema.md#block) table to get the block where the registration transaction was included.

`SELECT ph.id, COALESCE(po.pool_name, ph."view") as pool_name, ...` selects the pool ID and uses `COALESCE` to get the pool name, prioritizing the one from [pool_offline_data](https://github.com/cardano-foundation/cf-ledger-sync/blob/main/docs/pages/schema.md#pool_offline_data). It selects the ticker name and the block's timestamp (as `creation_date`)
...
- [pool_hash](https://github.com/cardano-foundation/cf-ledger-sync/blob/main/docs/pages/schema.md#pool_hash) aliased to (ph), [pool_offline_data](https://github.com/cardano-foundation/cf-ledger-sync/blob/main/docs/pages/schema.md#pool_offline_data) aliased to (po), [pool_update](https://github.com/cardano-foundation/cf-ledger-sync/blob/main/docs/pages/schema.md#pool_update) aliased to (pu),  [tx](https://github.com/cardano-foundation/cf-ledger-sync/blob/main/docs/pages/schema.md#tx) and [block](https://github.com/cardano-foundation/cf-ledger-sync/blob/main/docs/pages/schema.md#block) aliased to (b) 
- In addition, we query the [pool_retire](https://github.com/cardano-foundation/cf-ledger-sync/blob/main/docs/pages/schema.md#pool_retire) (pr) which holds information about stake pools that have been retired or de-registered.
