...
class TxWitnessServiceImplTest {
    ...
    @Test
    void handleTxWitness_whenSamePairKeyAndSignature() {
        ...
        for (var txWitness : txWitnesses) {
            if (txWitness.getKey().equals("91274605d7f9a5df9b9b72aa3da9f946390cc30fbcd055d1fbec1d06797b6228")) {
                ...
                assertThat(Arrays.stream(txWitness.getIndexArr())).contains(0, 1, 2);
            } else {
                assertEquals(2, txWitness.getIndexArrSize());
                assertThat(Arrays.stream(txWitness.getIndexArr())).contains(0, 1);
            }
        }
    }

    @Test
    void handleTxWitness_whenDiffPairKeyAndSignature() {
        Map<String, Tx> txMap = new HashMap<>();
